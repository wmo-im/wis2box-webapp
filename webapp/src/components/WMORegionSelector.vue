<template>
    <div v-if="! errorMessage">
      <v-autocomplete
        ...
        item-title="name"
        item-value="id"
        label="WMO Regional Association"
        v-model="selected"
        :readonly="readonly"
        :hint="selected ? selected.description : 'Select WMO Regional Association'"
        persistent-hint
        return-object
        />
    </div>
...
<script>
  ...
  export default defineComponent({
    ...
    setup(props, {emit}){
      const apiUrl = "/code_lists/WMORegion.json";
